...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => { if (e.key === "Escape") closeOverlays(); };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [closeOverlays]);
  ...
  return (
    <>
      ...
      >
        <nav
          aria-label="Primary"
          className="mx-auto grid h-25 max-w-[1600px] grid-cols-3 items-center px-3 sm:px-6 md:h-30 lg:h-40 lg:px-7"
        >
          {/* Mobile: hamburger left */}
          <button
            type="button"
          ...
          <div className="hidden lg:flex items-center gap-8 justify-self-start">
            <a href="/" className="flex items-center">
              <img src="/Thathwamlogofinal.png" alt="Thathwam Sarees" className="h-24 w-auto rounded-sm" />
            </a>
            ...
            <ul className="flex items-center gap-7">
              ...
              >
                ...
                {isCollectionOpen && (
                  <div className="absolute top-full left-0 pt-2 z-50">
                    <div className="min-w-[180px] bg-white text-[#3F010C]  border border-gold/25 shadow-lg py-2">
                      <a
                        href="/products"
                        className="block px-4 py-2 text-[12px] font-semibold uppercase tracking-label  hover:text-gold hover:bg-white/5 transition-colors duration-200 border-b border-gold/15 mb-1"
                      >
                        View All
                      </a>
                      {categories.map((cat) => (
                        <a
                          key={cat._id}
                          href={`/products?category=${encodeURIComponent(cat.name)}`}
                          className="block px-4 py-2 text-[12px] uppercase tracking-label  hover:text-gold hover:bg-white/5 transition-colors duration-200"
                        >
                          {cat.name}
                        </a>
                      ))}
          ...
          {/* Middle column: mobile logo / desktop search bar */}
          <div className="col-start-2 flex items-center justify-center">
            <a href="/" className="flex items-center lg:hidden">
              <img src="/Thathwamlogofinal.png" alt="Thathwam Sarees" className="h-22 w-auto rounded-sm" />
            </a>

            <form onSubmit={handleSearchSubmit} className="hidden lg:flex items-center w-full max-w-md border border-gold/30 rounded-md overflow-hidden">
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search for sarees..."
                className="flex-1 bg-[#FFFFF6] px-4 py-2.5 text-sm text-black outline-none placeholder:text-black/40"
              />
              <button
                type="submit"
                aria-label="Search"
                className="bg-gold px-5 py-2.5 text-white hover:bg-gold/90 transition-colors duration-200 flex items-center justify-center shrink-0"
              >
                <Search size={18} />
              </button>
            </form>
          ...
          <div className="col-start-3 flex items-center justify-self-end gap-2 sm:gap-3">
            ...
            <a
              href="/cart"
              aria-label={`Cart, ${itemCount} items`}
              className={`relative p-1 transition-colors duration-300 ease-in-out ${iconToneClass}`}
            >
              <CartIcon />
              {itemCount > 0 && (
                <span className="absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-gold text-[11px] font-medium text-[#3F010C]">
                  {itemCount}
                </span>
              )}
            </a>
      ...
      >
        <div className="flex items-center justify-end px-6 pt-6 sm:px-10 sm:pt-8">
          <button
            ...
            onClick={() => setIsSearchOpen(false)}
            aria-label="Close search"
            className="p-1 mt-2 text-[#3F010C] transition-colors duration-300 ease-in-out"
          >
            <CloseIcon />
          </button>
        </div>
      ...
      >
        <div className="flex items-center justify-between px-6 pt-6">
          ...
          <button
            ...
            onClick={() => setIsMobileMenuOpen(false)}
            aria-label="Close menu"
            className="p-1 text-[#FFFFF6]/80 transition-colors duration-300 ease-in-out"
          >
            <CloseIcon />
          </button>
        </div>
          ...
          <ul className="space-y-6">
            <li>
              <a
                href="/"
                onClick={() => setIsMobileMenuOpen(false)}
                className="font-serif text-3xl font-normal text-[#FFFFF6] hover:text-gold transition-colors duration-300 ease-in-out sm:text-4xl"
              >
                Home
              </a>
            </li>
            ...
            <li>
              <span className="font-serif text-3xl font-normal text-[#FFFFF6] sm:text-4xl">Collection</span>
              <ul className="mt-4 space-y-3 pl-2">
                <li>
                  <a
                    href="/products"
        ...
        <div className="flex items-center gap-8 border-t border-[#FFFFF6]/15 px-8 py-8">
          <a
            href="/cart"
            className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-label text-[#FFFFF6]/80 hover:text-gold transition-colors duration-300 ease-in-out"
          >
            <CartIcon width="16" height="16" />
            Cart ({itemCount})
          </a>
